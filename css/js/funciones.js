document.addEventListener("DOMContentLoaded", function () {
	...
	$("#codproducto").autocomplete({
        ...
        select: function (event, ui) {
            $("#codproducto").val(ui.item.value);
            setTimeout(
                function () {
                    e = jQuery.Event("keypress");
                    e.which = 13;
					registrarDetalleReq(e, ui.item.id, 2, ui.item.precio);
                }
            )
        }
    })
...
function listarReq() {
    ...
    $.ajax({
        ...
        success: function (response){
            document.querySelector("#detalle_ventaReq").innerHTML = html;
            calcular();
...
function registrarDetalleReq(e, id, cant, precio) {
    if (document.getElementById('codproducto').value != '') {
        if (e.which == 13) {
            if (id != null) {
                let action = 'regDetalleReq';
                $.ajax({
                    url: "ajax.php",
                    type: 'POST',
                    dataType: "json",
                    ...
                    success: function (response) {
                        if (response == 'registrado') {
                            ...
                            document.querySelector("#codproducto").value = '';
                            document.querySelector("#codproducto").focus();
                            listarReq();
                        } else if (response == 'actualizado') {
                            Swal.fire({
                                position: 'top-end',
