...
class Empresas
{
	...
	function readletraRif ($l)
    {
		...
		state, ciudad FROM " . $this->table_name . " where estado=1 and left(numero_rif, 1) = '$l' order by razon_social";
		//echo "<br>=== " . $query;
		try {			 
			$stmt = $this->conn->prepare ( $query );
			$stmt->execute ();
	...
	function readRif ($l)
    {
		$query = "SELECT id, numero_rif, razon_social, email_empresa, telefono_empresa, direccion_empresa, contacto_nombre, admin_nombre,
		state, ciudad FROM " . $this->table_name . " where estado=1 and numero_rif = '$l' order by razon_social";
		//echo "<br>=== " . $query;
		try {			 
			$stmt = $this->conn->prepare ( $query );
			$stmt->execute ();
	...
	function readtodoEmpresa ()
    {
		 try {			 
			$query = "SELECT id, razon_social, email_empresa, telefono_empresa, direccion_empresa, state, ciudad, plan FROM " . 
	...
	function cambiarEstatusEmpresa ($idCompany, $accion)
    {
		 try {			 
